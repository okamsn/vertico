...
;; This package provides a set of sort functions for Vertico,
;; which can be used as `vertico-sort-function' or
...
(defun vertico-sort--history-hash ()
  ...
  (or (and (equal (car vertico-sort--history-hash) vertico--base) (cdr vertico-sort--history-hash))
...
The function is configured by BY, BSIZE, BINDEX, BPRED and PRED."
  `(defun ,(intern (mapconcat #'symbol-name `(vertico sort ,@by) "-")) (candidates)
     ...
     (let* ((buckets (make-vector ,bsize nil))
            ,@(and (eq (car by) 'history) '((hhash (vertico-sort--history-hash)) (hcands))))
       (dolist (% candidates)
